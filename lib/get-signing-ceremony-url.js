...
function getSigningCeremonyUrl ({
  ...
  authApi.login(loginOps, (error, loginInfo) => {
    ...
    if (loginInfo) {
      ...
      doc.setName(createdPdfFileName);
      doc.setDocumentId('1');
      ...
      tabs.setSignHereTabs(signHereTabs.map(signHereTab => {
        const signHere = new docusign.SignHere();

        signHere.setDocumentId(signHereTab.documentId.toString());
        signHere.setPageNumber(signHereTab.pageNumber.toString());
        signHere.setRecipientId(signHereTab.recipientId.toString());
        signHere.setXPosition(signHereTab.x.toString());
        signHere.setYPosition(signHereTab.y.toString());
      ...
      envDef.getRecipients().setSigners([signer]);
